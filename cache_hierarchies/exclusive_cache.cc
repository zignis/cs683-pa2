...
bool is_l1_cache(int cache_type)
{
    return cache_type == IS_L1D || cache_type == IS_L1I;
}

void invalidate_cache_entry(uint32_t cpu, uint64_t addr, bool l1, bool l2,
                            bool llc)
{
    if (l1)
    {
        ooo_cpu[cpu].L1I.invalidate_entry(addr);
        ooo_cpu[cpu].L1D.invalidate_entry(addr);
    }

    if (l2)
        ooo_cpu[cpu].L2C.invalidate_entry(addr);
    if (llc)
        uncore.LLC.invalidate_entry(addr);
}

void handle_invalidation(uint32_t cpu, int cache_type, uint64_t addr)
{
    if (is_l1_cache(cache_type))
    {
        invalidate_cache_entry(cpu, addr, false, true, true);
    }
    else if (cache_type == IS_L2C)
    {
        invalidate_cache_entry(cpu, addr, true, false, true);
    }
    else if (cache_type == IS_LLC)
    {
        invalidate_cache_entry(cpu, addr, true, true, false);
    }
}

void assert_block_absence_in_cache(uint32_t cpu, PACKET *pkt, bool l1, bool l2,
                                   bool llc)
{
    if (l1 && (ooo_cpu[cpu].L1I.check_hit(pkt) >= 0 ||
               ooo_cpu[cpu].L1D.check_hit(pkt) >= 0))
    {
        std::cerr << "[violation]: block is also present in L1";
        assert(0);
    }

    if (l2 && ooo_cpu[cpu].L2C.check_hit(pkt) >= 0)
    {
        std::cerr << "[violation]: block is also present in L2";
        assert(0);
    }

    if (llc && uncore.LLC.check_hit(pkt) >= 0)
    {
        std::cerr << "[violation]: block is also present in LLC";
        assert(0);
    }
}
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
    {
        ...
        if (cache_type == IS_L2C)
        {
            if (MSHR.entry[mshr_index].type == PREFETCH_TRANSLATION ||
                ...
                fill_packet_type = 1;
            else if ((MSHR.entry[mshr_index].type == LOAD ||
            ...
            cache_type == IS_STLB)
        {
            ...
            if (MSHR.entry[mshr_index].data == (UINT64_MAX >> LOG2_PAGE_SIZE))
            {
                ...
                else
                {
                    ...
                    // MSHR inflight and add to PTW.RQ Case 2 : Translation of L1D
                    // prefetch request merged (from read queue) - mark MSHR inflight and
                    // add to DTLB RQ

                    // if(MSHR.entry[mshr_index].l1_pq_index != -1 ||
                    // MSHR.entry[mshr_index].prefetch_translation_merged)
                    // //TRANSALTION_FROM_L1D merged with LOAD/RFO
                    //{
                    ...
                    /*else	//PREFETCH merged with LOAD/RFO
                      {
                      MSHR.entry[mshr_index].returned = INFLIGHT;
                    //cout << "PREFETCH_TRANSLATION merged with LOAD_TRANSLATION-instr_id:
                    "<<MSHR.entry[mshr_index].instr_id<<" prefetch_id:
                    "<<MSHR.entry[mshr_index].prefetch_id;
                    //cout << " address: " << hex << MSHR.entry[mshr_index].address << "
                    full_addr: "<< MSHR.entry[mshr_index].full_addr << " data: ";
                    //cout << MSHR.entry[mshr_index].data << " type: " << dec <<
                    MSHR.entry[mshr_index].type << " event_cycle: " <<
                    MSHR.entry[mshr_index].event_cycle << endl; if(lower_level)
                    lower_level->add_rq(&MSHR.entry[mshr_index]);
                    update_fill_cycle();
                    }*/
                }
            ...
            if (lower_level)
            {
                    ...
                    lower_level->get_size(2, block[set][way].address))
                {
                    ...
                    DP(if (warmup_complete[fill_cpu]) {
                        ...
                        cout << " lower level wq is full!" << " fill_addr: " << hex
                             << MSHR.entry[mshr_index].address;
        ...
        if (do_fill)
        {
            ...
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            handle_invalidation(cpu, cache_type, MSHR.entry[mshr_index].address);
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#ifdef PUSH_DTLB_PB
            ...
            {
                uint32_t victim_way;
                victim_way = ooo_cpu[fill_cpu].DTLB_PB.find_victim(
                    fill_cpu, MSHR.entry[mshr_index].instr_id, 0,
                    ooo_cpu[fill_cpu].DTLB_PB.block[0], MSHR.entry[mshr_index].ip,
                    MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
                ooo_cpu[fill_cpu].DTLB_PB.update_replacement_state(
                    fill_cpu, 0, way, MSHR.entry[mshr_index].full_addr,
                    MSHR.entry[mshr_index].ip,
                    ooo_cpu[fill_cpu].DTLB_PB.block[0][victim_way].full_addr,
                    MSHR.entry[mshr_index].type, 0);
                ooo_cpu[fill_cpu].DTLB_PB.fill_cache(0, victim_way,
                                                     &MSHR.entry[mshr_index]);
            }
#endif
            ...
            if (cache_type == IS_STLB)
            {
                if ((MSHR.entry[mshr_index].instruction == 0) &&
                    (MSHR.entry[mshr_index].l1_rq_index != -1))
                {
                    stlb_hints_to_l2++;
                    uint64_t phy_addr =
                        ((block[set][way].data << LOG2_PAGE_SIZE) |
                         (ooo_cpu[fill_cpu]
                              .L1D.RQ.entry[MSHR.entry[mshr_index].l1_rq_index]
                              .full_addr &
                          ((1 << LOG2_PAGE_SIZE) - 1))) >>
                        LOG2_BLOCK_SIZE;
                    // Neelu: Sending type as 6 so that L2C prefetcher can differentiate
                    // the STLB hints.
                    DP(if (warmup_complete[fill_cpu]) {
                        cout << "[" << NAME << "] " << __func__
                             << "sending stlb hint to L2: ";
                        cout << " phy_addr: " << hex << phy_addr;
                        cout << " ip: " << MSHR.entry[mshr_index].ip << endl;
                    });

                    uint32_t temp_metadata = ooo_cpu[fill_cpu].L2C.l2c_prefetcher_operate(
                        phy_addr << LOG2_BLOCK_SIZE, MSHR.entry[mshr_index].ip, 0, 6, 0);
                    float
                        l2c_mpki; // =
                                  // (ooo_cpu[fill_cpu].L2C.sim_access[fill_cpu][0]*1000)/(ooo_cpu[fill_cpu].num_retired);
                    if (warmup_complete[fill_cpu])
                        if (ooo_cpu[fill_cpu].num_retired -
                                ooo_cpu[fill_cpu].warmup_instructions >
                            0)
                            l2c_mpki = (ooo_cpu[fill_cpu].L2C.sim_miss[fill_cpu][0] * 1000) /
                                       (ooo_cpu[fill_cpu].num_retired -
                                        ooo_cpu[fill_cpu].warmup_instructions);
                        else if (ooo_cpu[fill_cpu].num_retired > 0)
                            l2c_mpki = (ooo_cpu[fill_cpu].L2C.sim_miss[fill_cpu][0] * 1000) /
                                       (ooo_cpu[fill_cpu].num_retired);
                    /*			if((((temp_metadata >> 17) & 1) |
                       ((temp_metadata >> 18) & 1)) == 1) getting_hint_from_l2++;*/
                    uncore.LLC.llc_prefetcher_operate(phy_addr << LOG2_BLOCK_SIZE,
                                                      MSHR.entry[mshr_index].ip, 0, 6,
                                                      temp_metadata);
                }
            }
#endif
            ...
            if ((cache_type == IS_L2C) && (MSHR.entry[mshr_index].type == PREFETCH) &&
                (MSHR.entry[mshr_index].fill_level == FILL_L2))
            {
                // Neelu: Modifying the metadata to include a set bit (17th lsb bit)
                // that will convey that this request is already translated. uint32_t
                // updated_metadata = MSHR.entry[mshr_index].pf_metadata | (1 << 16);
                // Neelu: Commenting this, because now, this bit will be set by the L2C
                // prefetcher in the cache_fill function. Hence, argument passed will
                // directly be MSHR packet's metadata instead of updated_metadata if the
                // bit is set.
                if (((MSHR.entry[mshr_index].pf_metadata >> 16) & 1) == 1)
                    ooo_cpu[fill_cpu].L1D.prefetch_line(
                        MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr,
                        MSHR.entry[mshr_index].full_addr, FILL_L1,
                        MSHR.entry[mshr_index].pf_metadata);
            }
            ...
            { //@v Responses to prefetch requests should
                     ...
                     (MSHR.entry[mshr_index].type != PREFETCH))
            {
                ...
                if (PROCESSED.occupancy <
                    PROCESSED.SIZE) // Neelu: Commenting for ideal L1 prefetcher i.e.
                                    // not sending to processed queue
                    PROCESSED.add_queue(&MSHR.entry[mshr_index]);
#endif
            }
            ...
            else
            {
                ...
                if (do_fill)
                {
                    ...
                    else if (cache_type == IS_L1D)
                    {
                        ...
                        // prefetcher. l1d_prefetcher_cache_fill(WQ.entry[index].full_addr,
                        // set, way, 0, block[set][way].address<<LOG2_BLOCK_SIZE,
...
void CACHE::handle_processed()
{
    ...
    if (tlb.PROCESSED.occupancy != 0)
    {
             ...
             current_core_cycle[cpu]))
        {
            ...
            if (tlb.PROCESSED.entry[index].l1_rq_index != -1)
            {
                    ...
                    (RQ.entry[rq_index].full_addr) >> LOG2_PAGE_SIZE)
                {
                    ...
                    else
                    {
                        ...
                        ooo_cpu[cpu].LQ.entry[RQ.entry[rq_index].lq_index].translated =
                            COMPLETED;
                        ITERATE_SET(merged, RQ.entry[rq_index].lq_index_depend_on_me,
                                    ooo_cpu[cpu].LQ.SIZE)
                        {
            ...
            // Neelu: Checking for l1_pq_index as well as L1I prefetching is turned on
            // and the corresponding translation requests are sent to ITLB.
            ...
            // Neelu: Commenting this assert as ITLB can have translation requests
            // from L1I prefetches.
            // assert(tlb.PROCESSED.entry[index].prefetch_translation_merged ==
            // false);
                            ...
                            RQ.SIZE)
                {
                         ...
                         (RQ.entry[other_rq_index].full_addr) >> LOG2_PAGE_SIZE))
                    {
                        ...
                        else
                        {
                            // Neelu: Marking the corresponding LQ entry as translated.
                            ooo_cpu[cpu]
                                .LQ.entry[RQ.entry[other_rq_index].lq_index]
                                .translated = COMPLETED;
                            ITERATE_SET(merged,
                                        RQ.entry[other_rq_index].lq_index_depend_on_me,
                                        ooo_cpu[cpu].LQ.SIZE)
                            {
                                ooo_cpu[cpu].LQ.entry[merged].translated = COMPLETED;
                            }
...
void CACHE::handle_read()
{
    if (cache_type == IS_L1D)
    {
        //	cout << "Handle read cycle: " << current_core_cycle[cpu] << "PQ
        // Occupancy: " << PQ.occupancy << endl;
    ...
    for (uint32_t i = 0; i < MAX_READ; i++)
    {
            ...
            (RQ.occupancy > 0))
        {
            ...
            { // read hit
                ...
                // PACKET *pkt = &RQ.entry[index];

                // if (is_l1_cache(cache_type))
                // {
                //     assert_block_absence_in_cache(read_cpu, pkt, false, true, true);
                // }
                // else if (cache_type == IS_L2C)
                // {
                //     assert_block_absence_in_cache(read_cpu, pkt, false, false, true);
                // }
                // else if (cache_type == IS_LLC)
                // {
                //     assert_block_absence_in_cache(read_cpu, pkt, false, true, false);
                // }
                ...
                if (cache_type == IS_ITLB)
                {
                    ...
                    // RQ.entry[index].instruction_pa = (va_to_pa(read_cpu,
                    // RQ.entry[index].instr_id, RQ.entry[index].full_addr,
                    // RQ.entry[index].address))>>LOG2_PAGE_SIZE; //block[set][way].data;
                ...
                {
                    // RQ.entry[index].data_pa = (va_to_pa(read_cpu,
                    // RQ.entry[index].instr_id, RQ.entry[index].full_addr,
                    // RQ.entry[index].address))>>LOG2_PAGE_SIZE;  //block[set][way].data;
                    RQ.entry[index].data_pa = block[set][way].data;
                    //@Vasudha: Perfect DTLB Prefetcher
                    /*auto it = temp_page_table.find(RQ.entry[index].full_addr >>
                      LOG2_PAGE_SIZE); assert(it != temp_page_table.end());
                ...
                else if (cache_type == IS_STLB)
                {
                    ...
                    // Neelu: Checking where in the cache hierarchy the corresponding data
                    // is present on STLB hits.
#ifdef CHECK_DATA_HIT_ON_STLB_HIT
                    PACKET to_check_hit;
                    if ((RQ.entry[index].instruction == 0) &&
                        (RQ.entry[index].l1_rq_index != -1))
                    {
                        to_check_hit.full_physical_address =
                            (block[set][way].data << LOG2_PAGE_SIZE) |
                            (ooo_cpu[read_cpu]
                                 .L1D.RQ.entry[RQ.entry[index].l1_rq_index]
                                 .full_addr &
                             ((1 << LOG2_PAGE_SIZE) - 1));
                        to_check_hit.address =
                            to_check_hit.full_physical_address >> LOG2_BLOCK_SIZE;
                ...
                if (cache_type == 0) // perfect-ITLB and baseline DTLB
                {
                    ...
                    // RQ.entry[index].full_addr, RQ.entry[index].address);
                ...
                if (RQ.entry[index].type == LOAD)
                {
                    ...
                    if (cache_type == IS_L1D)
                        l1d_prefetcher_operate(
                            RQ.entry[index].full_addr, RQ.entry[index].ip, 1,
                            RQ.entry[index].type,
                            RQ.entry[index].critical_ip_flag); // RQ.entry[index].instr_id);
                    else if ((cache_type == IS_L2C) &&
                             ...
                             (RQ.entry[index].type != PREFETCH_TRANSLATION) &&
                             (RQ.entry[index].type !=
                              TRANSLATION_FROM_L1D))
                    { // Neelu: for dense region, only
                      ...
                      // l2c_pref_operate as well.
                        l2c_prefetcher_operate(block[set][way].address << LOG2_BLOCK_SIZE,
                                               ...
                                               0, RQ.entry[index].critical_ip_flag);
                    }
                    else if (cache_type == IS_LLC)
                    {
                        cpu = read_cpu;
            ...
            { // read miss
                ...
                else
                {
                    ...
                    { // already in-flight miss
                             ...
                             RQ.entry[index].type != TRANSLATION_FROM_L1D))
                        {
                            ...
                            // assert(cache_type != IS_L1I);//@Vishal: L1I cache does not
                            // generate prefetch packet.
                ...
                if (miss_handled)
                {
                    ...
                    if (RQ.entry[index].type == LOAD)
                    {
                        ...
                        if (cache_type == IS_L1D)
                            l1d_prefetcher_operate(
                                RQ.entry[index].full_addr, RQ.entry[index].ip, 0,
                                RQ.entry[index].type,
                                RQ.entry[index]
                                    .critical_ip_flag); // RQ.entry[index].instr_id);
                        else if ((cache_type == IS_L2C) &&
                                 (RQ.entry[index].type != PREFETCH_TRANSLATION) &&
                                 (RQ.entry[index].instruction == 0) &&
                                 (RQ.entry[index].type != LOAD_TRANSLATION) &&
            ...
            { // prefetch hit
                ...
                if (PQ.entry[index].pf_origin_level < fill_level)
                {
                    if (cache_type == IS_L1D)
                    {
                        ...
                        assert(0);
                        l1d_prefetcher_operate(
                            PQ.entry[index].full_addr, PQ.entry[index].ip, 1, PREFETCH,
                            PQ.entry[index]
                                .critical_ip_flag); //, PQ.entry[index].prefetch_id);
                    }
                    else if ((cache_type == IS_L2C) &&
                             (RQ.entry[index].type != PREFETCH_TRANSLATION) &&
                             (RQ.entry[index].instruction == 0) &&
                             ...
                             (RQ.entry[index].type != TRANSLATION_FROM_L1D))
                    {
                        PQ.entry[index].pf_metadata = l2c_prefetcher_operate(
                            ...
                            PQ.entry[index]
                                .critical_ip_flag); // PQ.entry[index].prefetch_id);
                        if ((((PQ.entry[index].pf_metadata >> 17) & 1) |
                             ((PQ.entry[index].pf_metadata >> 18) & 1)) == 1)
                    ...
                    else if (cache_type == IS_STLB)
                    {
                        ...
                        DP(if (warmup_complete[PQ.entry[index].cpu]) {
                            cout << "[" << NAME << "_PQ] " << __func__
                                 << " prefetch_id: " << PQ.entry[index].prefetch_id
                                 << "from handle prefetch on prefetch hit" << "instruction "
                                 << PQ.entry[index].instruction
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (PQ.entry[index].pf_origin_level < fill_level)
                                {
                                    if (cache_type == IS_L1D)
                                        l1d_prefetcher_operate(
                                            PQ.entry[index].full_addr, PQ.entry[index].ip, 0,
                                            PREFETCH,
                                            PQ.entry[index]
                                                .critical_ip_flag); // PQ.entry[index].prefetch_id);
                                    else if ((cache_type == IS_L2C) &&
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
    ...
    if (cache_type == IS_DTLB_PB)
    {
        if (packet->data == 0)
        {
            cout << "Inside DTLB_PB, current = " << current_core_cycle[cpu]
...
int CACHE::check_hit(PACKET *packet)
{
    ...
    {
        cout << "[" << NAME << "] " << __func__ << " instr_id: " << packet->instr_id
             << " type: " << +packet->type << hex << " addr: " << packet->address;
        cout << " full_addr: " << packet->full_addr << dec;
        cout << " set: " << set << " way: " << match_way;
        cout << " event: " << packet->event_cycle
             << " cycle: " << current_core_cycle[cpu] << " cpu: " << cpu << endl;
    }
#endif
...
int CACHE::add_rq(PACKET *packet)
{
    ...
    if (index != -1)
    {
        ...
        if (packet->instruction)
        {
            ...
            //@Vishal: ITLB read miss request is getting merged with pending DTLB read
            // miss request. when completed send to both caches
            //@Vasudha: Done below
            // if(cache_type == IS_STLB)
            ...
            // Neelu: Commenting the following code and inserting modified code below
            // as L1I prefetching is turned on now.
            /*if(cache_type == IS_ITLB || cache_type == IS_DTLB || cache_type ==
              IS_STLB)
              {
        ...
        else
        {
            ...
            //@Vishal: DTLB read miss request is getting merged with pending ITLB read
            // miss request. when completed send to both caches
            //@Vasudha: Done below
            // if(cache_type == IS_STLB)
            //       RQ.entry[index].send_both_tlb = true;
    ...
    if (packet->instr_id == QTRACE_INSTR_ID)
    {
        ...
        cout << " type: " << +RQ.entry[index].type << " head: " << RQ.head
             << " tail: " << RQ.tail << " occupancy: " << RQ.occupancy;
        cout << " event: " << RQ.entry[index].event_cycle
             << " current: " << current_core_cycle[RQ.entry[index].cpu]
             << " cpu: " << cpu << endl;
    }
#endif
...
int CACHE::add_wq(PACKET *packet)
{
    ...
    if (packet->instr_id == QTRACE_INSTR_ID)
    {
        ...
        cout << " data: " << hex << WQ.entry[index].data << dec;
        cout << " event: " << WQ.entry[index].event_cycle
             << " current: " << current_core_cycle[WQ.entry[index].cpu]
             << " cpu: " << cpu << endl;
    }
#endif
...
int CACHE::prefetch_line(
    uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int pf_fill_level,
    uint32_t prefetch_metadata) /*, uint64_t prefetch_id)*/ // Neelu: commented.
{
    //	if(cache_type == IS_L2C)
    //		cout<<"Aye Aye, Captain, requested.";

    // Neelu: Todo: So, do all prefetches access STLB, even the same page ones? Is
    // that correct?
    pf_requested++;
    DP(if (warmup_complete[cpu]) {
    ...
    if (PQ.occupancy < PQ.SIZE)
    {
        ...
        pf_packet.ip = ip;
        // pf_packet.prefetch_id = prefetch_id;		Neelu: commented,
        // Vasudha was using for debugging. Assigning to zero for now.
                             ...
                             uint32_t prefetch_metadata)
{
    ...
    if (PQ.occupancy < PQ.SIZE)
    {
        ...
        if (return_val > -2) //@Vishal: In some cases, even if the PQ is empty,
...
int CACHE::add_pq(PACKET *packet)
{
    ...
    if (index != -1)
    {
        ...
        //@Vasudha: Fails when DTLB prefetch with instructions 0, STLB prefetch with
        // instruction 0 and STLB prefetch with instruction 1 gets merged
    ...
    // Neelu: Not adding any addition condition for skipping translation for
    // prefetches pushed from L2 to L1 because full_phy_addr != 0.
    ...
    if (cache_type == IS_L1D && packet->full_physical_address == 0)
    {
        ...
        pf_requested++;
        pf_issued++;
        auto ppage_check = ooo_cpu[packet->cpu].PTW.page_table.find(
            packet->full_virtual_address >> LOG2_PAGE_SIZE);
...
{
    return check_nonfifo_queue(&MSHR, packet,
    ...
    for (index = 0; index < queue->SIZE; index++)
    {
        if (queue->entry[index].address == 0)
        {
            ...
            if (packet->instr_id == QTRACE_INSTR_ID)
            {
                ...
                cout << " index: " << index << " occupancy: " << MSHR.occupancy
                     << " cpu: " << cpu << endl;
            }
#endif
