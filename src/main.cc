...
//	{
	...
	if (context_switch_file == NULL) {
        ...
        index=0;
        while ((fscanf(context_switch_file, "%ld %hhd %hhd", &cs_file[index].cycle, &cs_file[index].swap_cpu[0], &cs_file[index].swap_cpu[1])) != EOF) {
...
//	}
	...
	cout<<"Reg RAW dependents: "<<ooo_cpu[i].reg_RAW_dependents <<endl;
    ...
    cout << "Average branch resolution latency (in cycles): " << (float) total_branch_resolution_latency / branch_resolution_count << endl;
    }
