...
uint32_t SCHEDULING_LATENCY = 0, EXEC_LATENCY = 0, DECODE_LATENCY = 1;
...
uint64_t total_branch_resolution_latency = 0,
            branch_resolution_count = 0,
            cycle_at_mispred_instr = 0;
			...
			if (IFETCH_BUFFER.entry[ifetch_buffer_index].is_branch) {
				...
				uint8_t branch_prediction = predict_branch(IFETCH_BUFFER.entry[ifetch_buffer_index].ip);
	    			if(IFETCH_BUFFER.entry[ifetch_buffer_index].branch_taken != branch_prediction)
	   			{
                    ...
                    cycle_at_mispred_instr = current_core_cycle[cpu];
...
//Neelu: Addition end
	...
	else
	  {
        ...
        if(arch_instr.instr_id == DEBUG_INSTRUCTION)
            cout << "instr_id: " << arch_instr.instr_id << " in read_from_trace in cycle: " << current_core_cycle[cpu] << endl;
...
uint32_t O3_CPU::add_to_ifetch_buffer(ooo_model_instr *arch_instr)
{
    ...
    if(arch_instr->instr_id == DEBUG_INSTRUCTION)
            cout << "instr_id: " << arch_instr->instr_id << " in add_to_ifetch_buffer in cycle: " << current_core_cycle[cpu] << " at index: " << index << " IF head: " << IFETCH_BUFFER.head << endl;
  ...
  if((fetch_stall == 1) && (current_core_cycle[cpu] >= fetch_resume_cycle) && (fetch_resume_cycle != 0))
	...
	for(uint32_t i=0; i<IFETCH_BUFFER.SIZE; i++)
	{
		...
		if((IFETCH_BUFFER.entry[index].fetched == 0))
		{
			...
			PACKET fetch_packet;
            if(IFETCH_BUFFER.entry[index].instr_id == DEBUG_INSTRUCTION)
                cout << "instr_id: " << IFETCH_BUFFER.entry[index].instr_id <<  " sent to instruction fetch in cycle: " << current_core_cycle[0] << endl;
			...
			if(rq_index != -2)
			{
				...
				for(uint32_t j=0; j<IFETCH_BUFFER.SIZE; j++)
				{
					if(((IFETCH_BUFFER.entry[j].ip)>>6) == ((IFETCH_BUFFER.entry[index].ip)>>6))
					{
                        ...
                        if(IFETCH_BUFFER.entry[j].instr_id == DEBUG_INSTRUCTION)
                            cout << "instr_id: " << IFETCH_BUFFER.entry[j].instr_id << " getting marked TRANSLATED completed in cycle: " << current_core_cycle[cpu] << endl;
					}
				}
			}
		}
	...
	for(uint32_t i=0; i<FETCH_WIDTH; i++)
	{
		...
		if((IFETCH_BUFFER.entry[IFETCH_BUFFER.head].translated == COMPLETED) && (IFETCH_BUFFER.entry[IFETCH_BUFFER.head].fetched == COMPLETED))
		{
			...
			{
                if(IFETCH_BUFFER.entry[IFETCH_BUFFER.head].instr_id == DEBUG_INSTRUCTION)
                    cout << "instr_id: " << IFETCH_BUFFER.entry[IFETCH_BUFFER.head].instr_id << " added to Decode Buffer in cycle: " << current_core_cycle[0] << endl;
	  ...
	  for(uint32_t i=0; i<DECODE_BUFFER.SIZE; i++)
	  {
			...
			if(DECODE_BUFFER.entry[DECODE_BUFFER.head].btb_miss == 1 && DECODE_BUFFER.entry[DECODE_BUFFER.head].branch_mispredicted == 0)
		{
			...
			if(branch_type == BRANCH_DIRECT_JUMP || branch_type == BRANCH_DIRECT_CALL || (branch_type == BRANCH_CONDITIONAL))
			{
				if(warmup_complete[cpu])
				{
                    total_branch_resolution_latency += current_core_cycle[cpu] - cycle_at_mispred_instr;
                    branch_resolution_count++;
					fetch_resume_cycle = current_core_cycle[cpu] + 1; //Resume fetch from next cycle.
                    // cout << "instr_id is: " << DECODE_BUFFER.entry[DECODE_BUFFER.head].instr_id << endl;
                    // assert(0);
				}
            ...
            if(DECODE_BUFFER.entry[DECODE_BUFFER.head].instr_id == DEBUG_INSTRUCTION)
                cout << "instr_id: " << DECODE_BUFFER.entry[DECODE_BUFFER.head].instr_id << " added to ROB in cycle: " << current_core_cycle[0] << " at rob_index: " << ROB.tail << " ROB head: " << ROB.head << " ROB head entry: " << ROB.entry[ROB.head].instr_id << " ROB tail: " << ROB.tail << " ROB occupancy: " << ROB.occupancy << endl;
...
void O3_CPU::schedule_instruction()
{
    ...
    if (ROB.head < ROB.tail)
    {
        for (uint32_t i = ROB.head; i < ROB.tail; i++)
        {
            ...
            else if(ROB.entry[i].reg_ready == 0)
                num_searched++;
    ...
    {
        for (int i = ROB.head; i < ROB.SIZE; i++)
        {
            if ((ROB.entry[i].fetched != COMPLETED) || /*(ROB.entry[i].event_cycle > current_core_cycle[cpu]) ||*/ (num_searched >= SCHEDULER_SIZE))
                return;
...
void O3_CPU::do_scheduling(uint32_t rob_index)
{
    ...
    if(ROB.entry[rob_index].instr_id == DEBUG_INSTRUCTION)
        cout << "instr_id: " << ROB.entry[rob_index].instr_id << " scheduled in cycle: " << current_core_cycle[0] << endl;
    ...
    if (ROB.entry[rob_index].is_memory)
        ...
        if (ROB.entry[rob_index].reg_ready) {
            ...
            if(ROB.entry[rob_index].instr_id == DEBUG_INSTRUCTION)
                cout << "instr_id: " << ROB.entry[rob_index].instr_id << " added to RTE1 entry: " << RTE1_tail << " and RTE1_head is: " << RTE1_head << " in cycle: " << current_core_cycle[0] << endl;
...
void O3_CPU::do_execution(uint32_t rob_index)
{
    //if (ROB.entry[rob_index].reg_ready && (ROB.entry[rob_index].scheduled == COMPLETED) && (ROB.entry[rob_index].event_cycle <= current_core_cycle[cpu])) {
        if(ROB.entry[rob_index].instr_id == DEBUG_INSTRUCTION)
                cout << "instr_id: " << ROB.entry[rob_index].instr_id << " started execution in ROB in cycle: " << current_core_cycle[0] << endl;
...
void O3_CPU::operate_lsq()
{
    ...
    while (load_issued < LQ_WIDTH) {
        if (RTL0[RTL0_head] < LQ_SIZE) {
            ...
            if(LQ.entry[RTL0[RTL0_head]].instr_id == DEBUG_INSTRUCTION)
                    cout << "instr_id: " << LQ.entry[RTL0[RTL0_head]].instr_id << " started execution in operate_lsq in cycle: " << current_core_cycle[0] << endl;
            ...
            if (ROB.entry[rob_index].branch_mispredicted)
	      	{
				if(warmup_complete[cpu])
				{
                    ...
                    // cout << "instr_id is: " << ROB.entry[rob_index].instr_id << endl;
                    // assert(0);
				}
...
void O3_CPU::reg_RAW_release(uint32_t rob_index)
{
    ...
    if(ROB.entry[rob_index].instr_id == DEBUG_INSTRUCTION)
        cout << __func__ << " called for: " << ROB.entry[rob_index].instr_id << endl;
            ...
            if (ROB.entry[rob_index].registers_index_depend_on_me[j].search (i)) {
                ...
                if (ROB.entry[i].num_reg_dependent == 0) {
                    ...
                    if (ROB.entry[i].is_memory)
                        ...
                        if(ROB.entry[i].instr_id == DEBUG_INSTRUCTION)
                            cout << "instr_id: " << ROB.entry[i].instr_id << " added to RTE0 entry: " << RTE0_tail << " and RTE0_head is: " << RTE0_head << " in cycle: " << current_core_cycle[0] << endl;
...
//Neelu: Marking IFETCH_BUFFER entries translated and fetched and then returning. 
     ...
     for(uint32_t j=0; j<IFETCH_BUFFER.SIZE; j++)
     {
	     if(((IFETCH_BUFFER.entry[j].ip)>>6) == ((complete_ip)>>6))
	     {
            ...
            if(IFETCH_BUFFER.entry[j].instr_id == DEBUG_INSTRUCTION)
                cout << "instr_id: " << IFETCH_BUFFER.entry[j].instr_id << " completed fetch instruction in cycle: " << current_core_cycle[cpu] << endl;
	     }
     }
        if(queue->entry[index].instr_id == DEBUG_INSTRUCTION)
                cout << "instr_id: " << queue->entry[index].instr_id << " completed fetch instruction in cycle: " << current_core_cycle[cpu] << endl;
     // remove this entry
